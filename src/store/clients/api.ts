...
export const fetchClients = (actions: AppActions) =>
  cfetch(`${process.env.REACT_APP_SQUARELET_API_URL}/clients/`, GET)
    ...
    .then(data => Promise.all([actions.upsertClients(data.results)]))
    .catch(error => {
...
export const updateClient = (client: Client, actions: AppActions) => {
  ...
  return cfetch(
    `${process.env.REACT_APP_SQUARELET_API_URL}/clients/${client.id}/`,
    PATCH(formData)
  )
    .then(checkAuth(actions))
    .then(response =>
...
export const createClient = (client: Client, actions: AppActions) => {
  ...
  return (
    cfetch(
      `${process.env.REACT_APP_SQUARELET_API_URL}/clients/`,
      POST(formData)
    )
      .then(checkAuth(actions))
